    ...
    try {
      ...
      switch (item.type) {
        case 'note':
          ...
          break
        default: {
          logger.info('Calling knowledgeBase.add with item:', JSON.stringify(sourceItem, null, 2))
          result = await window.api.knowledgeBase.add({
            base: baseParams,
            item: sourceItem,
            userId: userId
          })
          logger.info('knowledgeBase.add result:', result)
          break
        }
      }
      ...
      if (result) {
        const canonicalContent = (result as LoaderReturn & { content?: KnowledgeItem['content'] }).content
      ...
      EventEmitter.emit(EVENT_NAMES.KNOWLEDGE_BASE_ITEM_PROCESSED, {
        baseId,
        itemId: item.id,
        type: item.type
      })
