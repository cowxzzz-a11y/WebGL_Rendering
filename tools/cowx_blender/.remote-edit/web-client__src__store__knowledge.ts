    ...
    updateItemProcessingStatus(
      ...
      if (base) {
        ...
        if (item) {
          ...
          item.processingMessage = action.payload.message
          item.processingError = action.payload.error
          item.retryCount = action.payload.retryCount
          if (action.payload.serverManaged !== undefined) {
            item.serverManaged = action.payload.serverManaged
          }
        }
      }
    },
      ...
      if (base) {
        base.items.forEach((item) => {
          if (item.processingStatus === 'completed' || item.processingStatus === 'failed') {
            ...
            item.processingMessage = undefined
